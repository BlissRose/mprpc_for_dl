...
void send_request(int thread_id, std::atomic<int> &success_count, std::atomic<int> &fail_count)
{
    ...
    MprpcController controller; // 创建控制器对象，用于处理 RPC 调用过程中的错误
    ...
    { // 如果调用成功
        ...
        {                                                                                  // 检查响应中的错误码
            std::cout << "rpc login response success:" << response.sucess() << std::endl;  // 打印成功信息
...
int main(int argc, char **argv)
{
    ...
    // 输出统计结果
    std::cout << "Total requests: " << thread_count * requests_per_thread << std::endl;
    std::cout << "Success count: " << success_count << std::endl;
    std::cout << "Fail count: " << fail_count << std::endl;
    std::cout << "Elapsed time: " << elapsed.count() << " seconds" << std::endl;
    std::cout << "QPS: " << (thread_count * requests_per_thread) / elapsed.count() << std::endl;
